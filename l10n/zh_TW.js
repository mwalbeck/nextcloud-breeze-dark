OC.L10N.register(
    ...
    "This setting will enforce the usage of this theme. Please note, this will prevent users from disabling or choosing a different theme." : "此設定將會強制使用此佈景主題。請注意，這將會阻止使用者停用或選擇不同的佈景主題。",
    "Enforce use of the Breeze Dark theme" : "強制使用 Breeze Dark 佈景主題",
    "This setting will enable the automated activation by the clients system settings by default, for any unauthenticated users and users who have not set a preference." : "預設情況下，此設定會為任何未經身份驗證的使用者與未設定偏好的使用者啟用客戶端系統設定自動啟用。",
    "Enable Breeze Dark automated activation by the clients system settings by default" : "預設情況下透過客戶端系統設定啟用 Breeze Dark 自動啟用",
    "This setting will allow you to choose if the login page should be themed. This only has an effect if the theme is enforced or during the login process for a user that has the theme enabled." : "此設定讓您可以選擇是否要在登入頁面套用主題。此設定僅會在強制執行佈景主題或登入期間啟用佈景主題的使用者時生效。",
